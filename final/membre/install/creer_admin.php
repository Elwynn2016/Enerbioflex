...
if(!empty($_POST['creer_admin'])) {
	...
	if(!empty($login) AND !empty($prenom) ANd !empty($nom) AND !empty($email) AND !empty($passe_1) AND !empty($passe_2)) {
		if($passe_1 === $passe_2) {
			include($folder.'membre/function.php');
			$pass = Cryptage::crypter($passe_1);
			$resultat = Bdd::connectBdd()->prepare(INSERT.MEMBREZ."(`login`,`mdp`, `mail`, `nom`, `prenom`, `activation`, `niveau`) VALUES ('".$login."', '".$pass."', '".$email."', '".$nom."', '".$prenom."', '1', '3');");
			$resultat -> execute();
			$acces = Bdd::connectBdd()->prepare(INSERT.ACCESPROFILZ."(`id_membre`, `email`, `pays`, `ville`, `genre`, `naissance`, `nom`, `prenom`) VALUES ('1', '1', '1', '1', '1', '1', '1', '1');");
			$acces -> execute();
...
<form method="post" action="creer_admin.php" id="form">
    	...
    	<table align="center" width="450">
            ...
            <tr>
                ...
                <td><input type="password" name="passe_1" size="30" /></td>
            </tr>
            <tr>
            	<td align="right">Encore le Mot de passe : </td>
                <td><input type="password" name="passe_2" size="30" /></td>
            </tr>
            <tr>
